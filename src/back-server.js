...
const personsList = [];
let categoriesJSONList = isCategoryMode
  ? {}
  : Object.values(require('./src/dist/quotesCategories'));
let personsJSONList = isPersonMode
  ? {}
  : Object.values(require('./src/dist/quotesPersons'));
...
const createQuotesCategories = (categoryName) => {
  const categoryIndex = categoriesList.findIndex(
    (c) => c.name === removeBreakLines(categoryName).toLowerCase().trim()
  );
  if (categoryIndex > -1) {
    return;
  }
...
const getCategoryId = (categoryName) => {
  const category = categoriesJSONList.find(
...
const createQuotesPersons = (personName) => {
  const personIndex = personsList.findIndex(
    (c) => c.name === removeBreakLines(personName).toLowerCase().trim()
  );
  if (personIndex > -1) {
    return;
  }
...
const getPersonId = (personName) => {
  const category = personsJSONList.find(
    (c) => c.name.toLowerCase().trim() === personName.toLowerCase().trim()
  );
  if (!category) {
    return -1;
  }
...
(async () => {
  ...
  if (isCategoryMode && !isPersonMode) {
    categoriesJSONList = Object.values(categoriesJSONList).sort((a, b) => {
      if (a.name < b.name) {
        return -1;
      }
      if (a.name > b.name) {
